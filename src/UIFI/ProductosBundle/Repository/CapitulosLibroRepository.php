...
class CapitulosLibroRepository extends EntityRepository implements IReportableRepository
{
    ...
    */
    public function getCountAllByYear(){
      ...
      $sql = 'SELECT COUNT(e.anual) AS cantidad, e.anual AS fecha FROM  capitulosLibro e
              GROUP BY e.anual';
      $em = $this->getEntityManager();
    ...
    */
    public function getCountByGrupo( $code ){
      $em = $this->getEntityManager();
      $connection = $em->getConnection();
      $sql = 'SELECT COUNT(DISTINCT(l.id)) as cantidad, g.nombre, l.anual
              FROM grupo g, integrante i,grupo_integrante gi,capitulosLibro l, integrantes_capituloslibro il
              WHERE g.id = :code
              AND gi.grupo_id = g.id
              AND gi.integrante_id = i.id
              AND l.grupo = g.id
              AND il.integrante_id = i.id
              AND il.capituloslibro_id = l.id';
    ...
    */
    public function getCantidadByIntegrante($code,$idGrupo){
      ...
      $query = "SELECT  count(*) as numero
        FROM integrante i , grupo g, grupo_integrante gi, capitulosLibro l, integrantes_capituloslibro il
        WHERE i.id= :code
        AND gi.grupo_id = g.id
        AND g.id = :idGrupo
        AND gi.integrante_id = i.id
